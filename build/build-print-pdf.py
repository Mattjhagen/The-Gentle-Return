#!/usr/bin/env python3
"""
Build Camera-Ready KDP Paperback PDF (6x9) for The Gentle Return
Adheres to Amazon KDP interior specifications (G202145060)
"""
...
# Margins for 300-500 page book
GUTTER = 0.75 * inch      # Inside margin
OUTER = 0.625 * inch      # Outside margin
TOP_MARGIN = 0.75 * inch
BOTTOM_MARGIN = 0.75 * inch

USABLE_WIDTH = PAGE_WIDTH - GUTTER - OUTER  # 4.625 inch
USABLE_HEIGHT = PAGE_HEIGHT - TOP_MARGIN - BOTTOM_MARGIN # 7.5 inch
...
class NumberedCanvas(canvas.Canvas):
    ...
    def draw_page_decorations(self, page_count):
        ...
        # Don't draw page numbers or headers on first 4 pages (Half Title, Title, Copyright, Dedication)
        if page_num <= 4:
            return
        ...
        if is_odd:
            left_m = GUTTER
            right_m = PAGE_WIDTH - OUTER
        else:
            left_m = OUTER
            right_m = PAGE_WIDTH - GUTTER

        center_x = (left_m + right_m) / 2.0
...
def build_pdf():
    ...
    recto_frame = Frame(
        GUTTER, BOTTOM_MARGIN, USABLE_WIDTH, USABLE_HEIGHT,
        id='recto', topPadding=0, bottomPadding=0, leftPadding=0, rightPadding=0
    )
    verso_frame = Frame(
        OUTER, BOTTOM_MARGIN, USABLE_WIDTH, USABLE_HEIGHT,
        id='verso', topPadding=0, bottomPadding=0, leftPadding=0, rightPadding=0
    )

    doc.addPageTemplates([
        PageTemplate(id='Odd', frames=[recto_frame]),
        PageTemplate(id='Even', frames=[verso_frame]),
    ])
